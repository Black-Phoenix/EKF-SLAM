...
def laser_measurement_model(ekf_state, landmark_id):
    ...
    zhat = np.array([np.sqrt((x_t - x) ** 2 + (y_t - y) ** 2), slam_utils.clamp_angle(np.arctan2(y_t - y, x_t - x) - p)])
...
def laser_update(trees, assoc, ekf_state, sigmas, params):
    ...
    for i, pair in enumerate(assoc):
        if pair == -1:
            ...
            ekf_state['P'] = np.matmul(np.eye(ekf_state['P'].shape[0]) - np.matmul(K, H), ekf_state['P'])  # + 10**-6*np.eye(3)
    ekf_state['P'] = fix_cov(ekf_state['P'])
    return ekf_state
